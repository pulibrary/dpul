...
module StubbedManifestsHelper
  def stub_manifest(url:, fixture:, status: 200)
    stub_request(:head, url)
      .to_return(status:, body: "", headers: { 'content-type' => 'application/ld+json' })
    stub_request(:get, url)
      .to_return(status:, body: File.read(Rails.root.join("spec", "fixtures", "manifests", fixture)), headers: { 'content-type' => 'application/ld+json' })
  end
  ...
  def stub_metadata(id:, status: 200, fixture: nil, auth_token: nil)
    ...
    stub_request(:get, url)
      .to_return(status:, body: File.read(Rails.root.join("spec", "fixtures", "metadata", "#{fixture || id}.json")), headers: { 'content-type' => 'application/ld+json' })
  end

  def stub_ocr_content(id:, text:)
    allow(FiggyGraphql).to receive(:get_ocr_content_for_id).with(id:).and_return([text])
  end
